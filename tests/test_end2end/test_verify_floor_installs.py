...
# First data-operations release compatible with the current code's deferred_binding=True /
# record_match_rejection calls, which need mloda>=0.11.0, transitively required via
# mloda-community-data-operations; 0.4.4 is also where the time_bucketization helper move landed.
_DATA_OPERATIONS_FIRST_RELEASE = (0, 4, 4)
...
def test_data_operations_floors_point_at_a_released_version() -> None:
    """0.4.4 is the first release compatible with the current code's deferred_binding=True /
    record_match_rejection calls, which need mloda>=0.11.0."""
    ...
    for pair in pairs:
        assert version_tuple(pair.floor) >= _DATA_OPERATIONS_FIRST_RELEASE, (
            ...
            "compatible with the current deferred_binding=True/record_match_rejection calls (needing "
            "mloda>=0.11.0) is 0.4.4, so an install at the floor could not import"
        )
