...
forbidden by the CFW-backend rule, so they reject EMA up-front with a clear
``ValueError``. Compute subclasses implement ``_compute_ema`` (the backend
EWM) and ``_assert_source_column_present`` (the guard).
"""
