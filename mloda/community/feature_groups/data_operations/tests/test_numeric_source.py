"""Structural single-source-of-truth guards for the shared numeric-source module.

These tests guard the "exactly one place" Definition of Done for issue #214:
the per-backend "what counts as numeric" logic lives in exactly one module
(``data_operations/numeric_source.py``) and is imported by BOTH the point- and
scalar-arithmetic families from that single source.
...
dataset and flow through ``numeric_source`` after the refactor.
"""
...
class TestNumericSourceSingleSourceOfTruth:
    """Guard that numeric-source logic lives in one place, imported by both families."""
    ...
    def test_duckdb_numeric_prefixes_content(self) -> None:
        ...
        for expected in ("TINYINT", "BIGINT", "HUGEINT", "FLOAT", "DOUBLE", "DECIMAL", "NUMERIC", "BIGNUM"):
            assert expected in prefixes, f"{expected!r} missing from numeric_source.DUCKDB_NUMERIC_PREFIXES"
    ...
    def test_duckdb_backend_modules_do_not_redeclare_prefixes(self) -> None:
        ...
        for path in (
            ...
            assert not hasattr(module, "_DUCKDB_NUMERIC_PREFIXES"), (
                ...
                "it must be removed in favour of numeric_source.DUCKDB_NUMERIC_PREFIXES."
            )
    ...
    def test_duckdb_backends_use_shared_descriptor(self) -> None:
        ...
        assert point.duckdb_non_numeric_descriptor is numeric_source.duckdb_non_numeric_descriptor
        assert scalar.duckdb_non_numeric_descriptor is numeric_source.duckdb_non_numeric_descriptor
    ...
    def test_sqlite_backends_use_shared_descriptor(self) -> None:
        ...
        assert point.sqlite_non_numeric_descriptor is numeric_source.sqlite_non_numeric_descriptor
        assert scalar.sqlite_non_numeric_descriptor is numeric_source.sqlite_non_numeric_descriptor
    ...
    def test_pandas_backends_use_shared_descriptor(self) -> None:
        ...
        assert point.pandas_non_numeric_descriptor is numeric_source.pandas_non_numeric_descriptor
        assert scalar.pandas_non_numeric_descriptor is numeric_source.pandas_non_numeric_descriptor
    ...
    def test_polars_backends_use_shared_descriptor(self) -> None:
        ...
        assert point.polars_non_numeric_descriptor is numeric_source.polars_non_numeric_descriptor
        assert scalar.polars_non_numeric_descriptor is numeric_source.polars_non_numeric_descriptor
    ...
    def test_pyarrow_backends_use_shared_descriptor(self) -> None:
        ...
        assert point.pyarrow_non_numeric_descriptor is numeric_source.pyarrow_non_numeric_descriptor
        assert scalar.pyarrow_non_numeric_descriptor is numeric_source.pyarrow_non_numeric_descriptor
