...
class TestReferenceRankNoneAndNanOrderBy:
    ...
    ``PythonDictRank``'s DEFECT E (``TestPythonDictRankNonFloatNanHangs``), fixed the same
    way: unconditionally increment ``pos`` after ``run_start = pos`` and use a NaN-safe
    equality helper. These tests pin the fix and that None/NaN tie into one run, matching
