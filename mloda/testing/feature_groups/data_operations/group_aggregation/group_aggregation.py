"""Shared test base class, data, and helpers for group aggregation tests.
...
Group aggregation reduces to one row per unique group key. With
partition_by=["region"], the 12 input rows collapse to 4 output rows
(A, B, C, None).

The ``GroupAggregationTestBase`` class provides concrete test methods
...
class GroupAggregationTestBase(ABC):
    ...
    def test_sum_grouped_region(self) -> None:
        """Sum of value_int grouped by region."""
        fs = make_feature_set("value_int__sum_grouped", ["region"])
        ...
        result_col = self.extract_column(result, "value_int__sum_grouped")
    ...
    def test_avg_grouped_region(self) -> None:
        """Average of value_int grouped by region."""
        fs = make_feature_set("value_int__avg_grouped", ["region"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)
        ...
        region_col = self.extract_column(result, "region")
        result_col = self.extract_column(result, "value_int__avg_grouped")
        result_map = _build_result_map(region_col, result_col)
    ...
    def test_count_grouped_region(self) -> None:
        """Count of non-null value_int grouped by region."""
        fs = make_feature_set("value_int__count_grouped", ["region"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)
        ...
        region_col = self.extract_column(result, "region")
        result_col = self.extract_column(result, "value_int__count_grouped")
        result_map = _build_result_map(region_col, result_col)
        for region, expected in EXPECTED_COUNT_BY_REGION.items():
            assert result_map[region] == expected, f"region={region}"

    def test_min_grouped_region(self) -> None:
        """Minimum of value_int grouped by region."""
        fs = make_feature_set("value_int__min_grouped", ["region"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)
        ...
        region_col = self.extract_column(result, "region")
        result_col = self.extract_column(result, "value_int__min_grouped")
        result_map = _build_result_map(region_col, result_col)
        for region, expected in EXPECTED_MIN_BY_REGION.items():
            assert result_map[region] == expected, f"region={region}"

    def test_max_grouped_region(self) -> None:
        """Maximum of value_int grouped by region."""
        fs = make_feature_set("value_int__max_grouped", ["region"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)
        ...
        region_col = self.extract_column(result, "region")
        result_col = self.extract_column(result, "value_int__max_grouped")
        result_map = _build_result_map(region_col, result_col)
    ...
    def test_new_column_added(self) -> None:
        ...
        result_cols = self.extract_column(result, "value_int__max_grouped")
    ...
    def test_std_grouped_region(self) -> None:
        ...
        fs = make_feature_set("value_int__std_grouped", ["region"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)

        region_col = self.extract_column(result, "region")
        result_col = self.extract_column(result, "value_int__std_grouped")
        result_map = _build_result_map(region_col, result_col)
    ...
    def test_var_grouped_region(self) -> None:
        """Variance of value_int grouped by region."""
        self._skip_if_unsupported("var")
        fs = make_feature_set("value_int__var_grouped", ["region"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)

        region_col = self.extract_column(result, "region")
        result_col = self.extract_column(result, "value_int__var_grouped")
        result_map = _build_result_map(region_col, result_col)
        a_vals = [10, -5, 0, 20]
        a_var = sum((x - 6.25) ** 2 for x in a_vals) / (len(a_vals) - 1)
        assert result_map["A"] == pytest.approx(a_var, rel=1e-6)

    def test_median_grouped_region(self) -> None:
        """Median of value_int grouped by region."""
        self._skip_if_unsupported("median")
        fs = make_feature_set("value_int__median_grouped", ["region"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)

        region_col = self.extract_column(result, "region")
        result_col = self.extract_column(result, "value_int__median_grouped")
        result_map = _build_result_map(region_col, result_col)
    ...
    def test_mode_grouped_region(self) -> None:
        """Mode of value_int grouped by region."""
        self._skip_if_unsupported("mode")
        fs = make_feature_set("value_int__mode_grouped", ["region"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)

        region_col = self.extract_column(result, "region")
        result_col = self.extract_column(result, "value_int__mode_grouped")
        result_map = _build_result_map(region_col, result_col)
    ...
    def test_nunique_grouped_region(self) -> None:
        """Count of unique value_int values grouped by region."""
        self._skip_if_unsupported("nunique")
        fs = make_feature_set("value_int__nunique_grouped", ["region"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)
    ...
    def test_multi_key_partition_sum(self) -> None:
        ...
        fs = make_feature_set("value_int__sum_grouped", ["region", "category"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)
    ...
    def test_multi_key_partition_count(self) -> None:
        ...
        fs = make_feature_set("value_int__count_grouped", ["region", "category"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)
    ...
    def test_multi_key_float_avg(self) -> None:
        ...
        fs = make_feature_set("value_float__avg_grouped", ["region", "category"])
        result = self.implementation_class().calculate_feature(self.test_data, fs)

        region_col = self.extract_column(result, "region")
        category_col = self.extract_column(result, "category")
        result_col = self.extract_column(result, "value_float__avg_grouped")
        result_map = {(region_col[i], category_col[i]): result_col[i] for i in range(len(region_col))}
