...
class TestEveryEnterpriseManifestImportsWithoutTheWheel:
    ...
    def test_no_binary_plugin_id_is_installed_as_a_wheel(self) -> None:
        ...
        installed = [
            cls.BINARY_PLUGIN_ID for cls in licensed if importlib.util.find_spec(cls.BINARY_PLUGIN_ID) is not None
        ]
        assert not installed, (
            "installed as a real wheel, but the repo's suites assume the wheel is absent: uninstall it "
            f"or run only tests/test_binary_model_real/: {installed!r}"
        )
