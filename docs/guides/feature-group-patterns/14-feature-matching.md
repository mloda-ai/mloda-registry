...
- Both only see values in `Options`, including whatever a `PREFIX_PATTERN` capture binds into it before validation runs. A named capture binds unconditionally when its name matches a `PROPERTY_MAPPING` key; a legacy positional (unnamed) capture binds only when the value is already a member of that key's `allowed_values`. Once bound, `element_validator` and `match_guard` run against it like any explicit option and can reject it, with one exception: a bare membership check (no `element_validator` declared) can never reject a legacy-bound value, since membership already gated the binding. `match_guard` is skipped only when the option is absent from the merged options. Constrain the string path with the regex itself (see [Return Data Type Rule](../data-operation-patterns/16-return-data-type-rule.md)).
