...
- For `is_in`, the value must be a non-empty `list`, `tuple`, `set`, or `frozenset`; a string raises `ValueError`.
- A 2-element tuple like `("col", "equal")` is valid only for the `equal` operator and matches null and NaN rows.
