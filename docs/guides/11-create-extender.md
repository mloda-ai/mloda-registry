...
mloda terminates `MULTIPROCESSING` workers with no teardown, so a buffered sink loses events there: OpenLineage `async_http` or kafka transports, and an OTel `BatchSpanProcessor` or, for `OtelLogAuditSink`, `BatchLogRecordProcessor` installed by `child_bootstrap`. Use a synchronous sink instead under `MULTIPROCESSING`: OpenLineage `http`, `console`, or `file` transport (injected or via `OPENLINEAGE_CONFIG`), or OTel `SimpleSpanProcessor` (`SimpleLogRecordProcessor` for `OtelLogAuditSink`).
...
- `FEATURE_GROUP_MATCHED` reads `verified_context` at plan time, so `prepare`, `explain` and `diagnose` need the scope too, not only `run`. It writes a record only when it refuses, before any feature is calculated; without `fail_closed` a missing identity denies only at calculate and the run still proceeds. There is no allow event at match: the calculate record is the allow event.
...
Every record carries `policy_version`: the `policy_version` argument (a non-blank string), else a 12 hex character fingerprint of the constructor-supplied gate (sorted `required_identity` plus `fail_closed`). The default does not track code changes, so pin the package version for that. The key is additive within `record_version` 1.

`TeeAuditSink(*sinks)` writes each record to every sink in order; the first failure propagates and later sinks are skipped, so put the durable `NdjsonAuditSink` first. `OtelLogAuditSink()` (extra `mloda-enterprise[otel]`, which is `opentelemetry-api>=1.30,<2`; the package still loads without it and only constructing the sink raises `ImportError` naming the extra) emits one OpenTelemetry log record per audit record, so allow and deny decisions with the policy version reach a Collector. allow is INFO and deny is WARN, the body is the decision and the timestamp is the record's `event_time`. Attributes: `mloda.audit.decision`, `mloda.audit.deny_reason`, `mloda.audit.policy_version`, `mloda.audit.hook`, `mloda.run.id`, `mloda.tenant.id`, `mloda.project.id`, `user.hash`, `mloda.feature_group.name`, `mloda.feature.names`, `error.type`; blank or absent values are omitted.

`user.hash` is the sha256 hex of the principal (pseudonymous, not anonymous; hash the principal to join it to the NDJSON record), and `mloda.run.id` is the join key to `OtelExtender` spans. Data-access identities, feature values and exception messages are never included. Emitting is best effort: a failure is logged at WARNING and never fails the run. The sink looks up the global logger provider on every write and logs one WARNING per process while only the API default provider is installed. Under `MULTIPROCESSING` a spawned worker has no SDK logger provider unless `child_bootstrap` installs one (see [Pickle Compatibility](#pickle-compatibility)), and a synchronous exporter blocks the calculation thread. The OTel copy is not sealed or tamper-evident: the NDJSON log and its manifest stay the retained record, so compose them:
