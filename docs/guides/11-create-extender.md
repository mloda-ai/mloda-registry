...
The `data_access_identity` that core supplies on the context comes from the reader's `data_access_identity(data_access)` classmethod (see `BaseInputData.data_access_identity`), which is default-deny: a mapping's key names, a URI's `scheme://host/path` without user information (query and fragment dropped; an Azure storage container is kept), an existing local path as given, else the type name. A reader may override it to publish more, and owns what it publishes.
...
`resolve_data_access_identity` prefers core's raw `args[0]`, but falls back to core's identity (the reader's default-deny one, often just the type name) when `args[0]` is scheme-shaped (`://`) with a malformed scheme (leading whitespace, a digit-leading scheme, ...) that differs from it. `OtelExtender` also skips `mloda.data_access.identity` when the resolved identity contains `=`, `;` or whitespace, so a keyword DSN is dropped; `mloda.data_access.format` is unaffected.
