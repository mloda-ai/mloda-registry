...
| [audit_extender.py](https://github.com/mloda-ai/mloda-registry/blob/main/mloda/enterprise/extenders/audit/audit_extender.py) | Tenant-scoped audit record per calculation with an identity presence gate; a sink failure after a successful calculation fails the run by default; `seal_ndjson_runs` seals each run after it ends into a signed, hash-chained manifest that `verify_ndjson_log` checks (`mloda-enterprise-audit`, license required) |
