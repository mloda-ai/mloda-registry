...
**Where**: The tables below are generated by `scripts/check_framework_support_matrix.py`. Do not edit the block between the `BEGIN GENERATED` and `END GENERATED` markers directly. Run `python scripts/check_framework_support_matrix.py` after changing any `supported_*()` override.
...
Every ✗ in the detail tables corresponds to one of the three divergence kinds in [Known divergences](known-divergences.md):

- **Excluded op** (SQLite `upper` / `lower` / `reverse`, SQLite `percentile`, SQLite offset `diff` / `pct_change`, PyArrow aggregation/window `median` / `mode`): the implementation refuses to match at resolution time, and the test class's `supported_*()` override mirrors the refusal so inherited tests skip cleanly.
- **Missing framework** (PyArrow `frame_aggregate`, `offset`, `percentile`, `rank`): no production implementation exists. The operation requires native rolling / LAG / percentile / rank that PyArrow does not provide, and the reference implementation lives in pure Python over PyArrow arrays. Adding it requires a real framework implementation, not just relaxing an exclusion.
- **Tolerance not excluded, but behaviour constrained**: float-accumulation tolerance is not exposed as a ✗; it shows up as `use_approx=True` on the relevant cross-framework assertions. See the "Float accumulation order" entry in Known divergences.
