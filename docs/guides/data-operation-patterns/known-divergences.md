...
Every entry above carries a `<!-- machine-checked ... -->` block right under its heading, with `operation`, `framework`, `condition`, `mitigation_location`, and `regression_test` fields. `mloda/community/feature_groups/data_operations/tests/test_known_divergences.py` runs in `tox` and asserts (a) every entry has such a block with all fields populated, (b) every `regression_test` reference resolves to a real, importable test, and (c) every `mitigation_location` path exists. A new entry without a resolvable guard fails the build, so the doc cannot rot silently. Use repo-relative paths and `path.py::Class[::method]` (or `path.py::test_function`) for the test reference.
