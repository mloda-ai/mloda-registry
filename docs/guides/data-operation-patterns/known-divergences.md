...
| Implementation guard | Row-preserving implementations tag rows with helper columns (to restore input order after a reordering window function). A user column with the same name would collide silently. | The SQL backends (DuckDB, SQLite) pick a collision-free helper name at runtime via `pick_helper_column_name`, so they accept any user column name. The pandas/polars/pyarrow plugins still use hardcoded helpers, so they call `assert_no_reserved_columns()`; any input column starting with the reserved `__mloda_` prefix raises a clear `ValueError`. |
...
- **Operations (still guarded)**: `aggregation` (Polars Lazy), `frame_aggregate` (Pandas, Polars Lazy), `offset` (Pandas, Polars Lazy), `percentile` (Polars Lazy), `point_arithmetic` (all frameworks; applied at the base class for cross-backend consistency even though no implementation currently adds helper columns), `rank` (Polars Lazy), `scalar_aggregate` (Polars Lazy), `scalar_arithmetic` (all frameworks; applied at the base class for cross-backend consistency even though no implementation currently adds helper columns), `window_aggregation` (Pandas, Polars Lazy, PyArrow).
- **Operations (no longer guarded)**: the DuckDB and SQLite plugins (`binning`, `datetime`, `frame_aggregate`, `offset`, `rank`, `string`, `window_aggregation`) pick a collision-free helper name at runtime with `pick_helper_column_name` (`sql_utils`), so they accept any user column name and no longer call the guard.
- **Where it lives**: `mloda/community/feature_groups/data_operations/reserved_columns.py` defines `RESERVED_PREFIX = "__mloda_"` and the `assert_no_reserved_columns()` validator. Each still-guarded implementation calls it as the first statement of its `_compute_*` method.
...
- **Native framework behavior**: The pandas/polars/pyarrow implementations add an internal helper column (for example to record original row order before a reordering window function) with a hardcoded name. If an input already carries a column with the same name, the helper would either overwrite user data or be dropped silently. The SQL backends avoid this entirely by generating a guaranteed-unique helper name.
- **Mitigation kind**: Implementation guard (non-SQL backends) / collision-free naming (SQL backends).
- **How**: for the non-SQL backends, `assert_no_reserved_columns(<input columns>, framework=..., operation=...)` runs first in the guarded `_compute_*` method; any name starting with `__mloda_` raises `ValueError` with the offending name, framework, and operation. For the SQL backends, `pick_helper_column_name(taken=set(data.columns) | {feature_name})` returns the lowest `__mloda_rnN__` name not already present, so no collision is possible.
- **Regression signal**: `mloda/community/feature_groups/data_operations/tests/test_reserved_columns.py` tests the guard helper. The shared `ReservedColumnsTestMixin` runs per framework: SQL backend test classes set `reserved_columns_enforced() = False` and assert a reserved-prefixed user column is accepted; non-SQL classes keep asserting it is rejected.
