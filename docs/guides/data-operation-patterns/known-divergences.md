...
condition: native mode() breaks ties differently from the reference's first-occurrence rule
mitigation_location:
...
- **Regression signal**: The canonical fixture has values that tie; `test_cross_framework_agg[mode]` and `test_cross_framework[mode]` compare against the reference via `_compare_with_reference`.
...
regression_test:
- mloda/testing/feature_groups/data_operations/row_preserving/rank/rank.py::RankTestBase::test_none_and_nan_order_by_rank
- mloda/testing/feature_groups/data_operations/row_preserving/rank/rank.py::RankTestBase::test_none_and_nan_order_by_dense_rank
- mloda/testing/feature_groups/data_operations/row_preserving/rank/rank.py::RankTestBase::test_none_and_nan_order_by_percent_rank
-->
...
- **Where it lives**: `mloda/community/feature_groups/data_operations/python_dict_helpers.py` (`is_null_like`, `nulls_last_sort_key`, `order_values_equal`), consumed by `PythonDictRank._apply_rank`. `ReferenceRank` uses its own local NaN-safe tiering, not PythonDict's helpers, so it stays an independent oracle.
- **Reference behavior**: PyArrow >= 25.0's `pc.rank` ranks NaN as a distinct value ahead of null under any `null_placement` (PyArrow < 25.0 tied them); DuckDB's `RANK()` and Polars' `.rank()` do the same. On `order_by = [None, nan, None, 1.0]` all three, and `ReferenceRank`, give `[3, 2, 3, 1]`.
- **Cross-framework check**: PythonDict, pandas and SQLite give `[2, 2, 2, 1]` (None and NaN tied). Pandas ties because a float64 column cannot distinguish NaN from null; SQLite ties because the sqlite3 driver coerces NaN to NULL on ingest, so `RANK()` never sees a NaN. PythonDict is the only backend that can represent both and chooses to tie them.
- **Mitigation kind**: Accepted divergence (no mitigation attempted).
- **How**: `nulls_last_sort_key` and `order_values_equal` fold NaN and `None` into one null-like tier (`is_null_like`) on purpose: this fixes a tie-run-splitting bug where a NaN row could sort between two None rows and break the run. Splitting them would trade agreement with pandas for agreement with PyArrow 25/DuckDB/Polars, a lateral move, and reopen that bug, so the divergence is documented instead of mitigated.
- **Regression signal**: `RankTestBase.ranks_none_and_nan_apart()` is `True` by default; `TestPythonDictRank`, `TestPandasRank` and `TestSqliteRank` override it to `False`. The three shared tests `test_none_and_nan_order_by_rank` / `_dense_rank` / `_percent_rank` in `mloda/testing/feature_groups/data_operations/row_preserving/rank/rank.py` pin the reference at `[3, 2, 3, 1]` and each backend at its own choice, so a change in any backend's or the reference's tie behavior fails that backend's run. `ReferenceRank` previously compared ties with plain `==` and hung on NaN (`nan == nan` is `False`, so the tie loop never advanced); its local tiering fixed that.
