...
- `tests/test_return_data_type_rule_invariants.py::test_completeness` enumerates, for every base class that declares a deterministic output type, a representative feature for each deterministic op and asserts the rule returns the expected non-`None` `DataType`. Any extraction regression that would have silently returned `None` now fails CI here. That set of base classes is derived from the family registry rather than hand-listed, so a new family with a deterministic rule and no representative feature fails instead of going uncovered.
