...
- **Supply chain**: `[tool.uv] exclude-newer = "7 days"` in `pyproject.toml` defers new dependency releases by 7 days. The `exclude-newer-package` exemptions for `mloda`, `uv`, and `mloda-example-binary` permit releases within that window because `mloda` and `mloda-example-binary` (the sibling `mloda-binary-wrapper` repo's wheel, an optional extra, see `docs/guides/feature-group-patterns/28-binary-backed-features.md`) are first-party and `uv` is the resolver itself. Those exempt packages rest on the hashes pinned in `uv.lock` and the gate's `--frozen` install. Do not edit this without a reason.
